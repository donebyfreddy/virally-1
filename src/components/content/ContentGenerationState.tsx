...
export function ContentGenerationState(props: Props) {
  ...
  useEffect(() => {
    if (!ACTIVE.has(props.status)) return;
    let delay = 2_500;
  ...
  const provider = props.jobs.find((job) => job.provider)?.provider ?? null;
  const model = props.jobs.find((job) => job.model)?.model ?? null;
  const completedJobs = props.jobs.filter((job) => job.status === "completed").length;
  ...
  const failedJob = [...props.jobs].reverse().find((job) =>
    ["failed", "dead_letter", "cancelled"].includes(job.status),
  );
  const errorMessage = props.error.message ?? failedJob?.failureMessage ?? "The generation could not be completed.";
  const errorCode = props.error.code ?? failedJob?.failureCode ?? "UNKNOWN_ERROR";
  const errorStage = props.error.stage ?? failedJob?.capability ?? stageLabel(failedJob?.type ?? null);
      ...
      {props.status === "failed" ? (
        <Card className="border-[var(--danger-mark)]">
          ...
          <CardBody className="flex flex-col gap-[var(--space-5)]">
            ...
            <AssetProgress completed={props.completedVisuals} total={props.totalVisuals} />
        ...
        <Card>
          <CardHeader
            as="h2"
            divided
            title={props.status === "planned" ? "Content plan ready" : "Generating your content"}
            action={ACTIVE.has(props.status) ? <LoaderCircle aria-hidden="true" size={17} className="animate-spin text-[color:var(--accent)]" /> : null}
          />
          <CardBody className="grid gap-[var(--space-6)] lg:grid-cols-[minmax(0,1fr)_17rem]">
            ...
            <dl className="grid content-start gap-[var(--space-4)] border-t border-[var(--border-subtle)] pt-[var(--space-5)] lg:border-l lg:border-t-0 lg:pl-[var(--space-6)] lg:pt-0">
              <Fact label="Current stage" value={currentStageLabel(props.status)} />
              <Fact label="Provider" value={provider ?? (props.status === "planned" ? "Selected at submission" : "Waiting for worker")} />
              <Fact label="Model" value={model ?? "—"} />
              <Fact label="Elapsed" value={elapsed} icon={<Clock3 aria-hidden="true" size={13} />} />
              <Fact label="Assets" value={`${completedJobs} of ${props.jobs.length} jobs complete`} />
